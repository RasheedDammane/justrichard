...
export default function DoctorForm({ locale, doctor }: DoctorFormProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  // Parse JSON arrays from doctor data
  const parseJsonArray = (data: any) => Array.isArray(data) ? data : [];
  const parseJsonObject = (data: any) => (data && typeof data === 'object') ? data : {};

  const [specializations, setSpecializations] = useState<string[]>(parseJsonArray(doctor?.specializations));
  const [tags, setTags] = useState<string[]>(parseJsonArray(doctor?.tags));
  const [languages, setLanguages] = useState<string[]>(parseJsonArray(doctor?.languages));
  const [certifications, setCertifications] = useState<string[]>(parseJsonArray(doctor?.certifications));
  const [achievements, setAchievements] = useState<string[]>(parseJsonArray(doctor?.achievements));
  const [availableDays, setAvailableDays] = useState<string[]>(parseJsonArray(doctor?.availableDays));
  const [doctoringFormats, setDoctoringFormats] = useState<string[]>(parseJsonArray(doctor?.doctoringFormats));
  const [targetAudience, setTargetAudience] = useState<string[]>(parseJsonArray(doctor?.targetAudience));
  const [clientLevels, setClientLevels] = useState<string[]>(parseJsonArray(doctor?.clientLevels));
  
  // Complex objects states
  const [programs, setPrograms] = useState<any[]>(parseJsonArray(doctor?.programs));
  const [education, setEducation] = useState<any[]>(parseJsonArray(doctor?.education));
  const [packagePricing, setPackagePricing] = useState<any[]>(parseJsonArray(doctor?.packagePricing));
  const [locations, setLocations] = useState<any[]>(parseJsonArray(doctor?.locations));
  const [workingHours, setWorkingHours] = useState<any>(parseJsonObject(doctor?.workingHours) || { start: '06:00', end: '21:00' });
  const [bookingTypes, setBookingTypes] = useState<string[]>(parseJsonArray(doctor?.bookingTypes));
  ...
  const [formData, setFormData] = useState({
    ...
    title: doctor?.title || '',
    bio: doctor?.bio || '',
    mainCategory: doctor?.mainCategory || '',
    experience: doctor?.experience || 0,
    sessionFee: doctor?.sessionFee || '',
    hourlyRate: doctor?.hourlyRate || '',
    currency: doctor?.currency || 'THB',
    ...
    image: doctor?.image || '',
    video: doctor?.video || '',
    totalClients: doctor?.totalClients || 0,
    ...
    acceptsOnline: doctor?.acceptsOnline ?? true,
    acceptsInPerson: doctor?.acceptsInPerson ?? true,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="fitness_doctoring">Fitness Doctoring</option>
              <option value="sport_doctoring">Sport Doctoring</option>
              <option value="nutrition_doctoring">Nutrition Doctoring</option>
              <option value="wellness_doctoring">Wellness Doctoring</option>
              <option value="life_doctoring">Life Doctoring</option>
              <option value="business_doctoring">Business Doctoring</option>
              <option value="career_doctoring">Career Doctoring</option>
            </select>
          </div>
      ...
      {/* Doctoring Formats & Target Audience */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">📋 Doctoring Formats & Target Audience</h3>
            ...
            <div className="space-y-2">
              {['In Person', 'Online', 'Private Session', 'Group Class', 'Semi-Private', 'Workshop'].map((format) => (
                <label key={format} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={doctoringFormats.includes(format)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setDoctoringFormats([...doctoringFormats, format]);
                      } else {
                        setDoctoringFormats(doctoringFormats.filter(f => f !== format));
                      }
                    }}
                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                  />
