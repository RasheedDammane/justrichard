...
export default function LegalProfessionalForm({ locale, legalProfessional }: LegalProfessionalFormProps) {
  ...
  // Parse JSON arrays from legalProfessional data
  const parseJsonArray = (data: any) => Array.isArray(data) ? data : [];
  const parseJsonObject = (data: any) => (data && typeof data === 'object') ? data : {};

  const [specializations, setSpecializations] = useState<string[]>(parseJsonArray(legalProfessional?.specializations));
  const [tags, setTags] = useState<string[]>(parseJsonArray(legalProfessional?.tags));
  const [languages, setLanguages] = useState<string[]>(parseJsonArray(legalProfessional?.languages));
  const [certifications, setCertifications] = useState<string[]>(parseJsonArray(legalProfessional?.certifications));
  const [achievements, setAchievements] = useState<string[]>(parseJsonArray(legalProfessional?.achievements));
  const [availableDays, setAvailableDays] = useState<string[]>(parseJsonArray(legalProfessional?.availableDays));
  const [legalProfessionalingFormats, setLegalProfessionalingFormats] = useState<string[]>(parseJsonArray(legalProfessional?.legalProfessionalingFormats));
  const [targetAudience, setTargetAudience] = useState<string[]>(parseJsonArray(legalProfessional?.targetAudience));
  const [clientLevels, setClientLevels] = useState<string[]>(parseJsonArray(legalProfessional?.clientLevels));
  
  // Complex objects states
  const [programs, setPrograms] = useState<any[]>(parseJsonArray(legalProfessional?.programs));
  const [education, setEducation] = useState<any[]>(parseJsonArray(legalProfessional?.education));
  const [packagePricing, setPackagePricing] = useState<any[]>(parseJsonArray(legalProfessional?.packagePricing));
  const [locations, setLocations] = useState<any[]>(parseJsonArray(legalProfessional?.locations));
  const [workingHours, setWorkingHours] = useState<any>(parseJsonObject(legalProfessional?.workingHours) || { start: '06:00', end: '21:00' });
  const [bookingTypes, setBookingTypes] = useState<string[]>(parseJsonArray(legalProfessional?.bookingTypes));
  
  const [formData, setFormData] = useState({
    name: legalProfessional?.name || '',
    slug: legalProfessional?.slug || '',
    title: legalProfessional?.title || '',
    bio: legalProfessional?.bio || '',
    mainCategory: legalProfessional?.mainCategory || '',
    experience: legalProfessional?.experience || 0,
    sessionFee: legalProfessional?.sessionFee || '',
    hourlyRate: legalProfessional?.hourlyRate || '',
    currency: legalProfessional?.currency || 'THB',
    cityId: legalProfessional?.cityId || '',
    countryId: legalProfessional?.countryId || '',
    email: legalProfessional?.email || '',
    phone: legalProfessional?.phone || '',
    website: legalProfessional?.website || '',
    instagram: legalProfessional?.instagram || '',
    facebook: legalProfessional?.facebook || '',
    image: legalProfessional?.image || '',
    video: legalProfessional?.video || '',
    totalClients: legalProfessional?.totalClients || 0,
    successRate: legalProfessional?.successRate || '',
    rating: legalProfessional?.rating || '',
    isActive: legalProfessional?.isActive ?? true,
    isFeatured: legalProfessional?.isFeatured ?? false,
    isVerified: legalProfessional?.isVerified ?? true,
    isAvailable: legalProfessional?.isAvailable ?? true,
    acceptsOnline: legalProfessional?.acceptsOnline ?? true,
    acceptsInPerson: legalProfessional?.acceptsInPerson ?? true,
    metaTitle: legalProfessional?.metaTitle || '',
    metaDescription: legalProfessional?.metaDescription || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = legalProfessional 
        ? `/api/admin/legalProfessionales/${legalProfessional.id}` 
        : '/api/admin/legalProfessionales';
      ...
      if (response.ok) {
        router.push(`/${locale}/admin/legalProfessionales`);
        router.refresh();
      } else {
        const error = await response.json();
        alert(error.message || 'Failed to save legalProfessional');
      }
    } catch (error) {
      console.error('Error saving legalProfessional:', error);
      alert('Error saving legalProfessional');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="fitness_legalProfessionaling">Fitness LegalProfessionaling</option>
              <option value="sport_legalProfessionaling">Sport LegalProfessionaling</option>
              <option value="nutrition_legalProfessionaling">Nutrition LegalProfessionaling</option>
              <option value="wellness_legalProfessionaling">Wellness LegalProfessionaling</option>
              <option value="life_legalProfessionaling">Life LegalProfessionaling</option>
              <option value="business_legalProfessionaling">Business LegalProfessionaling</option>
              <option value="career_legalProfessionaling">Career LegalProfessionaling</option>
            </select>
          </div>
      ...
      {/* LegalProfessionaling Formats & Target Audience */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">📋 LegalProfessionaling Formats & Target Audience</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* LegalProfessionaling Formats */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              LegalProfessionaling Formats
            </label>
            <div className="space-y-2">
              {['In Person', 'Online', 'Private Session', 'Group Class', 'Semi-Private', 'Workshop'].map((format) => (
                <label key={format} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={legalProfessionalingFormats.includes(format)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setLegalProfessionalingFormats([...legalProfessionalingFormats, format]);
                      } else {
                        setLegalProfessionalingFormats(legalProfessionalingFormats.filter(f => f !== format));
                      }
                    }}
                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                  />
      ...
      <div className="flex items-center justify-end gap-4 pt-6 border-t">
        ...
        >
          ...
          {loading ? 'Saving...' : 'Save LegalProfessional'}
        </button>
      </div>
    </form>
  );
