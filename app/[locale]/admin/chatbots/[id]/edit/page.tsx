...
export default function EditChatbotPage() {
  ...
  const fetchChatbot = async () => {
    try {
      ...
      if (response.ok && data.chatbot) {
        ...
        setFormData({
          name: chatbot.name || '',
          type: chatbot.type || 'CUSTOMER_SUPPORT',
          industry: chatbot.industry || 'GENERAL',
          partnerId: chatbot.partnerId || '',
        ...
        setIntents(chatbot.intents || []);
      }
    } catch (error) {
      console.error('Error fetching chatbot:', error);
      alert('Erreur lors du chargement du chatbot');
    } finally {
    ...
    try {
      const response = await fetch(`/api/admin/chatbots/${chatbotId}`, {
        ...
        body: JSON.stringify({
          ...
          intents: intents.map(({ id, ...intent }) => intent), // Remove id for update
        }),
      });

      if (response.ok) {
        router.push(`/${locale}/admin/chatbots/${chatbotId}`);
      } else {
        const error = await response.json();
        alert(error.error || 'Erreur lors de la mise à jour du chatbot');
      }
    } catch (error) {
      console.error('Error updating chatbot:', error);
      alert('Erreur lors de la mise à jour du chatbot');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm('Êtes-vous sûr de vouloir supprimer ce chatbot ? Cette action est irréversible.')) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        alert(error.error || 'Erreur lors de la suppression du chatbot');
      }
    } catch (error) {
      console.error('Error deleting chatbot:', error);
      alert('Erreur lors de la suppression du chatbot');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-primary-600 text-white py-8 px-4">
        <div className="container mx-auto max-w-4xl">
          <h1 className="text-3xl font-bold">Modifier le Chatbot</h1>
          <p className="text-primary-100 mt-1">Mettre à jour les informations du chatbot</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-between">
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => router.push(`/${locale}/admin/chatbots/${chatbotId}`)}
