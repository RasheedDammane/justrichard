...
export default function RentalCarForm({ locale, rentalCar }: RentalCarFormProps) {
  ...
  // Parse JSON arrays from rentalCar data
  const parseJsonArray = (data: any) => Array.isArray(data) ? data : [];
  const parseJsonObject = (data: any) => (data && typeof data === 'object') ? data : {};

  const [specializations, setSpecializations] = useState<string[]>(parseJsonArray(rentalCar?.specializations));
  const [tags, setTags] = useState<string[]>(parseJsonArray(rentalCar?.tags));
  const [languages, setLanguages] = useState<string[]>(parseJsonArray(rentalCar?.languages));
  const [certifications, setCertifications] = useState<string[]>(parseJsonArray(rentalCar?.certifications));
  const [achievements, setAchievements] = useState<string[]>(parseJsonArray(rentalCar?.achievements));
  const [availableDays, setAvailableDays] = useState<string[]>(parseJsonArray(rentalCar?.availableDays));
  const [rentalCaringFormats, setRentalCaringFormats] = useState<string[]>(parseJsonArray(rentalCar?.rentalCaringFormats));
  const [targetAudience, setTargetAudience] = useState<string[]>(parseJsonArray(rentalCar?.targetAudience));
  const [clientLevels, setClientLevels] = useState<string[]>(parseJsonArray(rentalCar?.clientLevels));
  
  // Complex objects states
  const [programs, setPrograms] = useState<any[]>(parseJsonArray(rentalCar?.programs));
  const [education, setEducation] = useState<any[]>(parseJsonArray(rentalCar?.education));
  const [packagePricing, setPackagePricing] = useState<any[]>(parseJsonArray(rentalCar?.packagePricing));
  const [locations, setLocations] = useState<any[]>(parseJsonArray(rentalCar?.locations));
  const [workingHours, setWorkingHours] = useState<any>(parseJsonObject(rentalCar?.workingHours) || { start: '06:00', end: '21:00' });
  const [bookingTypes, setBookingTypes] = useState<string[]>(parseJsonArray(rentalCar?.bookingTypes));
  ...
  const [formData, setFormData] = useState({
    ...
    bio: rentalCar?.bio || '',
    mainCategory: rentalCar?.mainCategory || '',
    experience: rentalCar?.experience || 0,
    sessionFee: rentalCar?.sessionFee || '',
    hourlyRate: rentalCar?.hourlyRate || '',
    currency: rentalCar?.currency || 'THB',
    cityId: rentalCar?.cityId || '',
    countryId: rentalCar?.countryId || '',
    email: rentalCar?.email || '',
    phone: rentalCar?.phone || '',
    website: rentalCar?.website || '',
    instagram: rentalCar?.instagram || '',
    facebook: rentalCar?.facebook || '',
    image: rentalCar?.image || '',
    video: rentalCar?.video || '',
    totalClients: rentalCar?.totalClients || 0,
    successRate: rentalCar?.successRate || '',
    rating: rentalCar?.rating || '',
    isActive: rentalCar?.isActive ?? true,
    isFeatured: rentalCar?.isFeatured ?? false,
    isVerified: rentalCar?.isVerified ?? true,
    isAvailable: rentalCar?.isAvailable ?? true,
    acceptsOnline: rentalCar?.acceptsOnline ?? true,
    acceptsInPerson: rentalCar?.acceptsInPerson ?? true,
    metaTitle: rentalCar?.metaTitle || '',
    metaDescription: rentalCar?.metaDescription || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = rentalCar 
        ? `/api/admin/rentalCares/${rentalCar.id}` 
        : '/api/admin/rentalCares';
      
      const method = rentalCar ? 'PUT' : 'POST';
      ...
      if (response.ok) {
        router.push(`/${locale}/admin/rentalCares`);
        router.refresh();
      } else {
        const error = await response.json();
        alert(error.message || 'Failed to save rentalCar');
      }
    } catch (error) {
      console.error('Error saving rentalCar:', error);
      alert('Error saving rentalCar');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="fitness_rentalCaring">Fitness RentalCaring</option>
              <option value="sport_rentalCaring">Sport RentalCaring</option>
              <option value="nutrition_rentalCaring">Nutrition RentalCaring</option>
              <option value="wellness_rentalCaring">Wellness RentalCaring</option>
              <option value="life_rentalCaring">Life RentalCaring</option>
              <option value="business_rentalCaring">Business RentalCaring</option>
              <option value="career_rentalCaring">Career RentalCaring</option>
            </select>
          </div>
      ...
      {/* RentalCaring Formats & Target Audience */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">📋 RentalCaring Formats & Target Audience</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* RentalCaring Formats */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              RentalCaring Formats
            </label>
            <div className="space-y-2">
              {['In Person', 'Online', 'Private Session', 'Group Class', 'Semi-Private', 'Workshop'].map((format) => (
                <label key={format} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={rentalCaringFormats.includes(format)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setRentalCaringFormats([...rentalCaringFormats, format]);
                      } else {
                        setRentalCaringFormats(rentalCaringFormats.filter(f => f !== format));
                      }
                    }}
                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                  />
