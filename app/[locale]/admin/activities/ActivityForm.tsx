...
export default function ActivityForm({ locale, activity }: ActivityFormProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  // Parse JSON arrays from activity data
  const parseJsonArray = (data: any) => Array.isArray(data) ? data : [];
  const parseJsonObject = (data: any) => (data && typeof data === 'object') ? data : {};

  const [specializations, setSpecializations] = useState<string[]>(parseJsonArray(activity?.specializations));
  const [tags, setTags] = useState<string[]>(parseJsonArray(activity?.tags));
  const [languages, setLanguages] = useState<string[]>(parseJsonArray(activity?.languages));
  const [certifications, setCertifications] = useState<string[]>(parseJsonArray(activity?.certifications));
  const [achievements, setAchievements] = useState<string[]>(parseJsonArray(activity?.achievements));
  const [availableDays, setAvailableDays] = useState<string[]>(parseJsonArray(activity?.availableDays));
  const [activityingFormats, setActivityingFormats] = useState<string[]>(parseJsonArray(activity?.activityingFormats));
  const [targetAudience, setTargetAudience] = useState<string[]>(parseJsonArray(activity?.targetAudience));
  const [clientLevels, setClientLevels] = useState<string[]>(parseJsonArray(activity?.clientLevels));
  
  // Complex objects states
  const [programs, setPrograms] = useState<any[]>(parseJsonArray(activity?.programs));
  const [education, setEducation] = useState<any[]>(parseJsonArray(activity?.education));
  const [packagePricing, setPackagePricing] = useState<any[]>(parseJsonArray(activity?.packagePricing));
  const [locations, setLocations] = useState<any[]>(parseJsonArray(activity?.locations));
  const [workingHours, setWorkingHours] = useState<any>(parseJsonObject(activity?.workingHours) || { start: '06:00', end: '21:00' });
  const [bookingTypes, setBookingTypes] = useState<string[]>(parseJsonArray(activity?.bookingTypes));
  ...
  const [formData, setFormData] = useState({
    ...
    totalClients: activity?.totalClients || 0,
    successRate: activity?.successRate || '',
    rating: activity?.rating || '',
    isActive: activity?.isActive ?? true,
    isFeatured: activity?.isFeatured ?? false,
    isVerified: activity?.isVerified ?? true,
    isAvailable: activity?.isAvailable ?? true,
    acceptsOnline: activity?.acceptsOnline ?? true,
    acceptsInPerson: activity?.acceptsInPerson ?? true,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="fitness_activitying">Fitness Activitying</option>
              <option value="sport_activitying">Sport Activitying</option>
              <option value="nutrition_activitying">Nutrition Activitying</option>
              <option value="wellness_activitying">Wellness Activitying</option>
              <option value="life_activitying">Life Activitying</option>
              <option value="business_activitying">Business Activitying</option>
              <option value="career_activitying">Career Activitying</option>
            </select>
          </div>
      ...
      {/* Activitying Formats & Target Audience */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">📋 Activitying Formats & Target Audience</h3>
            ...
            <div className="space-y-2">
              {['In Person', 'Online', 'Private Session', 'Group Class', 'Semi-Private', 'Workshop'].map((format) => (
                <label key={format} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={activityingFormats.includes(format)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setActivityingFormats([...activityingFormats, format]);
                      } else {
                        setActivityingFormats(activityingFormats.filter(f => f !== format));
                      }
                    }}
                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                  />
