...
import { Users, UserCheck, Star, MapPin, Phone, Mail, Plus, Gavel } from 'lucide-react';

export default async function AdminLawyersPage({ params: { locale } }: { params: { locale: string } }) {
  

  // Récupérer les providers (lawyers)
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage lawyers and legal professionals</p>
          </div>
          <Link
            href={`/${locale}/admin/lawyers/new`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
            ...
            <div className="text-sm opacity-90">Active Lawyers</div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-bold text-gray-900">Lawyers List</h2>
          </div>
          
          {items.length === 0 ? (
            <div className="p-12 text-center">
              <Gavel className="w-16 h-16 text-gray-300 mx-auto mb-4" />
              <p className="text-gray-500 text-lg mb-2">No lawyers found</p>
              <p className="text-gray-400 mb-6">Start by adding your first lawyer</p>
              <Link
                href={`/${locale}/admin/lawyers/new`}
                  ...
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <Link
                          href={`/${locale}/admin/lawyers/${item.id}`}
                          className="text-blue-600 hover:text-blue-900 mr-3"
                        >
                          View
                        </Link>
                        <Link
                          href={`/${locale}/admin/lawyers/${item.id}/edit`}
