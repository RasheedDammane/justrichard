...
export default function LawyersClient({ lawyers, stats, locale }: { lawyers: Provider[]; stats: any; locale: string }) {
  const t = useAdminTranslation('lawyers');
      ...
      <div className="bg-primary-600 text-white py-8 px-4">
        <div className="container mx-auto max-w-7xl">
          <div className="flex justify-between items-center">
            ...
            <Link href={`/${locale}/admin/lawyers/new`} className="bg-white text-primary-600 px-6 py-3 rounded-lg font-semibold hover:bg-primary-50 flex items-center gap-2">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('table.lawyer')}</th>
                  ...
                  <tr key={lawyer.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex items-center">
                        ...
                        {lawyer.ProviderLocation[0]?.City?.name || 'N/A'}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{lawyer._count.ProviderReview}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${lawyer.isActive ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                        {lawyer.isActive ? tc('active') : tc('inactive')}
                      </span>
                    </td>
