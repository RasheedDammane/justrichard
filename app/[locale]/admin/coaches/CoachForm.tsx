...
export default function CoachForm({ locale, coach }: CoachFormProps) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  // Parse JSON arrays from coach data
  const parseJsonArray = (data: any) => Array.isArray(data) ? data : [];
  const parseJsonObject = (data: any) => (data && typeof data === 'object') ? data : {};

  const [specializations, setSpecializations] = useState<string[]>(parseJsonArray(coach?.specializations));
  const [tags, setTags] = useState<string[]>(parseJsonArray(coach?.tags));
  const [languages, setLanguages] = useState<string[]>(parseJsonArray(coach?.languages));
  const [certifications, setCertifications] = useState<string[]>(parseJsonArray(coach?.certifications));
  const [achievements, setAchievements] = useState<string[]>(parseJsonArray(coach?.achievements));
  const [availableDays, setAvailableDays] = useState<string[]>(parseJsonArray(coach?.availableDays));
  const [coachingFormats, setCoachingFormats] = useState<string[]>(parseJsonArray(coach?.coachingFormats));
  const [targetAudience, setTargetAudience] = useState<string[]>(parseJsonArray(coach?.targetAudience));
  const [clientLevels, setClientLevels] = useState<string[]>(parseJsonArray(coach?.clientLevels));
  
  // Complex objects states
  const [programs, setPrograms] = useState<any[]>(parseJsonArray(coach?.programs));
  const [education, setEducation] = useState<any[]>(parseJsonArray(coach?.education));
  const [packagePricing, setPackagePricing] = useState<any[]>(parseJsonArray(coach?.packagePricing));
  const [locations, setLocations] = useState<any[]>(parseJsonArray(coach?.locations));
  const [workingHours, setWorkingHours] = useState<any>(parseJsonObject(coach?.workingHours) || { start: '06:00', end: '21:00' });
  const [bookingTypes, setBookingTypes] = useState<string[]>(parseJsonArray(coach?.bookingTypes));
  ...
  const [formData, setFormData] = useState({
    ...
    title: coach?.title || '',
    bio: coach?.bio || '',
    mainCategory: coach?.mainCategory || '',
    experience: coach?.experience || 0,
    sessionFee: coach?.sessionFee || '',
    hourlyRate: coach?.hourlyRate || '',
    currency: coach?.currency || 'THB',
    cityId: coach?.cityId || '',
    ...
    acceptsOnline: coach?.acceptsOnline ?? true,
    acceptsInPerson: coach?.acceptsInPerson ?? true,
    metaTitle: coach?.metaTitle || '',
    metaDescription: coach?.metaDescription || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        router.push(`/${locale}/admin/coaches`);
        router.refresh();
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="sport_coaching">Sport Coaching</option>
      ...
      {/* Coaching Formats & Target Audience */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">📋 Coaching Formats & Target Audience</h3>
            ...
            <div className="space-y-2">
              {['In Person', 'Online', 'Private Session', 'Group Class', 'Semi-Private', 'Workshop'].map((format) => (
                <label key={format} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={coachingFormats.includes(format)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setCoachingFormats([...coachingFormats, format]);
                      } else {
                        setCoachingFormats(coachingFormats.filter(f => f !== format));
                      }
                    }}
                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                  />
