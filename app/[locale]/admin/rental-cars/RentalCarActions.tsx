...
export default function RentalCarActions({ car, locale }: RentalCarActionsProps) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/admin/rental-cars/${car.id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="flex items-center gap-2">
      <Link
        href={`/${locale}/rental-cars/${car.slug}`}
      ...
      >
        {car.isActive ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
      </button>
