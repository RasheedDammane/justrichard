...
export default function MaidForm({ locale, maid }: MaidFormProps) {
  ...
  // Parse JSON arrays from maid data
  const parseJsonArray = (data: any) => Array.isArray(data) ? data : [];
  const parseJsonObject = (data: any) => (data && typeof data === 'object') ? data : {};

  const [specializations, setSpecializations] = useState<string[]>(parseJsonArray(maid?.specializations));
  const [tags, setTags] = useState<string[]>(parseJsonArray(maid?.tags));
  const [languages, setLanguages] = useState<string[]>(parseJsonArray(maid?.languages));
  const [certifications, setCertifications] = useState<string[]>(parseJsonArray(maid?.certifications));
  const [achievements, setAchievements] = useState<string[]>(parseJsonArray(maid?.achievements));
  const [availableDays, setAvailableDays] = useState<string[]>(parseJsonArray(maid?.availableDays));
  const [maidingFormats, setMaidingFormats] = useState<string[]>(parseJsonArray(maid?.maidingFormats));
  const [targetAudience, setTargetAudience] = useState<string[]>(parseJsonArray(maid?.targetAudience));
  const [clientLevels, setClientLevels] = useState<string[]>(parseJsonArray(maid?.clientLevels));
  
  // Complex objects states
  const [programs, setPrograms] = useState<any[]>(parseJsonArray(maid?.programs));
  const [education, setEducation] = useState<any[]>(parseJsonArray(maid?.education));
  const [packagePricing, setPackagePricing] = useState<any[]>(parseJsonArray(maid?.packagePricing));
  const [locations, setLocations] = useState<any[]>(parseJsonArray(maid?.locations));
  const [workingHours, setWorkingHours] = useState<any>(parseJsonObject(maid?.workingHours) || { start: '06:00', end: '21:00' });
  const [bookingTypes, setBookingTypes] = useState<string[]>(parseJsonArray(maid?.bookingTypes));
  ...
  const [formData, setFormData] = useState({
    ...
    title: maid?.title || '',
    bio: maid?.bio || '',
    mainCategory: maid?.mainCategory || '',
    experience: maid?.experience || 0,
    sessionFee: maid?.sessionFee || '',
    hourlyRate: maid?.hourlyRate || '',
    currency: maid?.currency || 'THB',
    cityId: maid?.cityId || '',
    countryId: maid?.countryId || '',
    email: maid?.email || '',
    phone: maid?.phone || '',
    website: maid?.website || '',
    instagram: maid?.instagram || '',
    facebook: maid?.facebook || '',
    image: maid?.image || '',
    video: maid?.video || '',
    totalClients: maid?.totalClients || 0,
    successRate: maid?.successRate || '',
    rating: maid?.rating || '',
    isActive: maid?.isActive ?? true,
    isFeatured: maid?.isFeatured ?? false,
    isVerified: maid?.isVerified ?? true,
    isAvailable: maid?.isAvailable ?? true,
    acceptsOnline: maid?.acceptsOnline ?? true,
    acceptsInPerson: maid?.acceptsInPerson ?? true,
    metaTitle: maid?.metaTitle || '',
    metaDescription: maid?.metaDescription || '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.message || 'Failed to save maid');
      }
    } catch (error) {
      console.error('Error saving maid:', error);
      alert('Error saving maid');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="fitness_maiding">Fitness Maiding</option>
              <option value="sport_maiding">Sport Maiding</option>
              <option value="nutrition_maiding">Nutrition Maiding</option>
              <option value="wellness_maiding">Wellness Maiding</option>
              <option value="life_maiding">Life Maiding</option>
              <option value="business_maiding">Business Maiding</option>
              <option value="career_maiding">Career Maiding</option>
            </select>
          </div>
      ...
      {/* Maiding Formats & Target Audience */}
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-gray-900">📋 Maiding Formats & Target Audience</h3>
            ...
            <div className="space-y-2">
              {['In Person', 'Online', 'Private Session', 'Group Class', 'Semi-Private', 'Workshop'].map((format) => (
                <label key={format} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={maidingFormats.includes(format)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setMaidingFormats([...maidingFormats, format]);
                      } else {
                        setMaidingFormats(maidingFormats.filter(f => f !== format));
                      }
                    }}
                    className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                  />
