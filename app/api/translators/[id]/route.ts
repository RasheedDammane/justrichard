 ...
 *     description: Retrieve detailed information about a specific translator
 *     parameters:
 *       - in: path
 *         name: id
 *         required: true
...
export async function GET(
  ...
  try {
    const item = await prisma.translator.findUnique({
      where: { id: params.id },
      include: {
        city: true,
      },
    ...
    if (!item) {
      return NextResponse.json(
        { success: false, error: 'Translator not found' },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to fetch translator' },
      { status: 500 }
    );
  }
}
 ...
 *     description: Update all fields of translator
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
...
export async function PUT(
  ...
  try {
    ...
    const item = await prisma.translator.update({
      where: { id: params.id },
      data: body,
      include: {
        city: true,
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to update translator' },
      { status: 500 }
    );
  }
}
 ...
 *   patch:
 *     tags: [Translation]
 *     summary: Partially update translator
 *     description: Update specific fields of translator
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
 ...
 *     description: Soft delete or permanently delete translator
 *     security:
 *       - bearerAuth: []
 *     parameters:
 *       - in: path
...
export async function DELETE(
  ...
  try {
    ...
    if (permanent) {
      await prisma.translator.delete({
        where: { id: params.id },
      });
    } else {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to delete translator' },
      { status: 500 }
    );
  }
}
